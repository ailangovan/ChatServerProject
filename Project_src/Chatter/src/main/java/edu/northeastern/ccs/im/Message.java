...
public class Message {
    ...
    /**
     * Checks if is logging message.
     *
     * @return true, if is logging message
     */
    public boolean isLoggingMessage() {
        return msgType == MessageType.LOGGING;
    }
	 ...
	 * @return the message
	 */
	public static Message makeLoggingMessage(String myName, String text) {
		return new Message(MessageType.LOGGING, myName, text);
	}
    ...
    protected static Message makeMessage(String handle, String srcName, String text) {
        ...
        } else if (handle.compareTo(MessageType.SUBPOENA.toString()) == 0) {
            result = makeSubpoenaMessage(srcName, text);
        }else if (handle.compareTo(MessageType.RECALL.toString()) == 0) {
		    result = makeRecallMessage(srcName, text);
        }else if (handle.compareTo(MessageType.LOGGING.toString()) == 0) {
		    result = makeLoggingMessage(srcName, text);
        }
        return result;
    }

    public static Message makeGroupModifyMessage(String myName, String text) {
        return new Message(MessageType.GROUPMODIFY, myName, text);
    }
    ...
    public static Message makeSubpoenaMessage(String myName, String text) {
        return new Message(MessageType.SUBPOENA, myName, text);
    }
    /**
    ...
    /**
     * Determine if this message is sent go user as USERMESSAGE
     *
     * @return True if the message is an USERMESSAGE message; false otherwise
     */
